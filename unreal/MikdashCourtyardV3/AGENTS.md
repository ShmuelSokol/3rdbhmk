...
## Structured resident source export / Study10 — 19 September 2026
build_resident_structured_source.py copies retained attribute and morph rows as
raw bytes from ResidentShoulderAdopt01. Only Tunic/Mantle indices change; compact
referenced vertex accessors preserve accepted sleeves, skin, UVs and morphs.
Original binary payload remains in GLB. Independent verifier confirmed both
exports: Elder45744 triangles, Woman_Young40164. Generator layout comparison
needs1e-12 tolerance for NORMAL morph deltas too (observed~1e-14 platform noise);
exported rows are still byte-exact, with no tolerance applied to preservation.
Study10 imports into its own namespace and skips further global simplification;
VAT uses the reduced skeletal source as driver, captures compare accepted full
skeletal source. Native results are recorded below. No runtime adoption.

Study10 Elder four-view inspection: no obvious new orange skin-through-robe or
belt-through-mantle defects. Shading remains different. Code inspection found
MikdashCrowdField disables cast shadows while the skeletal reference keeps them.
Capture adds opt-in ShadowParity (both false) with property readback to isolate
that confounder. Default scene/crowd behavior is unchanged. Matched trial results are recorded below;
do not attribute all shading differences to normals or claim this explains all.
...
## Full-clip native mesh lifetime — 19 September 2026
Study11 direct-normal bake initially allocated a DynamicMesh per frame. Deleting
the Python reference did not reclaim Unreal UObjects during the commandlet;
private memory grew from2.31GB at37 frames to3.46GB before completion. Stopped only
the verified owned process19508 before the4GiB cap; no candidate assets had yet
been saved. Reuse one DynamicMesh across every walk/idle frame. Keep all memory
guards unchanged. Initial partial walk export and terminal wrapper receipt remain
as failed-attempt evidence; retries use fresh timestamps and an empty namespace.
...
## Frozen idle/blend review controls — 19 September 2026
IdleOff also drives crowd palette brightness even with PaletteMix/SkinVariation=0.
For matched idle/reference review only, transient MIDs divide BaseColor by that
known brightness factor, with readback; production materials remain unchanged.
Assert commandlet world time is zero before/after captures. Blend states use the
actual BlendSeconds and switch-time custom data; fixed endpoints are not proof of
continuous transition motion or intermediate skeletal blend parity.
Render wrapper receipt discovery must match only render-<timestamp>.json, not
auxiliary render-*-direction-check.json. An auxiliary proof written during a native
job caused a false multi-output wrapper failure although UE exited0 and completed
Elder Rear idle captures. Preserve failed wrapper and reconcile its exact native
output; do not repeat a completed capture merely to obtain a green wrapper.
...
## Distance-stop probe settings — 19 September 2026
ClassifyByDistance requires0 < FreezeCm < CullCm; inverted/equal thresholds fall
back to simulation. Initial stop probe game-20260919T151507Z correctly froze48,
but requested Cull1 with Freeze30000, so culled0 and failed the harness assertion.
This was invalid test input, not a reservation failure. Retain its receipt/log.
The corrected probe sets Freeze1/Cull2 together, then restores both30000. Native
source/binaries unchanged; do not infer culling from the command alone: require
48 culled and final0 readbacks, frame spacing, and live movement after restoration.
