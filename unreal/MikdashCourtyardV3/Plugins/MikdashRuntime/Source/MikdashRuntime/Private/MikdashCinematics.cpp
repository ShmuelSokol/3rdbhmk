...
{
    // Sources, shot by shot, are in Scripts/release_intro_sequence.spec.json. In short:
    // the bus and its traced street level, the Kotel face midpoint and its traced terrain,
    // the Mount platform deck top, the outer court floor top, the outer perimeter wall
...
{
    return {0.00f, 0.34f, 0.58f, 0.82f, 1.00f};
}

float UMikdashCinematics::GetDurationSeconds() const
{
    // Clamped to the brief. A shot shorter than 40 seconds cannot establish the city and
