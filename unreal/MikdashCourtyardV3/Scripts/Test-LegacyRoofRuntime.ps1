...
$before=Map-Hashes
$r=[ordered]@{status='running';archive=$Archive;childSha256=$hash;nullRHI=[bool]$NullRHI;startedUtc=(Get-Date).ToUniversalTime().ToString('o');mapHashesBefore=$before;scope='Existing cp24 cooked assets plus verified replacement game binary; NOT a recook or stone-material update.'}
...
try {
    ...
    if ($process) {
        ...
        if (!$process.HasExited) {
            # Stop only the exact child this script launched, never another game/window.
            $live=Get-Process -Id $process.Id -ErrorAction SilentlyContinue
            if ($live -and $live.Path -eq $exe) { Stop-Process -Id $live.Id; $r.terminatedOwnedChild=$true }
        }
    }
