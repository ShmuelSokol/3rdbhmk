"""Fresh full-source Man_Standard VAT candidate; no map or live crowd adoption.

Run in hidden Entry editor through run_crowd_near_v1.ps1. Default only verifies
...
NS = '/Game/MikdashV3/Runtime/CrowdNearV1'
...
def main():
    ...
    try:
        ...
        if build:
            ...
            skel, walk, idle = [ue.load_asset(variant[k]) for k in ('skeletalMesh', 'walk', 'idle')]
            assert skel and walk and idle
            ...
            master = vat.build_master_v2(ue, spec, NS, 'M_CrowdNear_V1', defaults, report)
            ...
            row.update(mesh=mesh.get_path_name(), textures={k: v.get_path_name() for k,v in textures.items()},
                       stats=vat._mesh_stats(ue, mesh), slots=slots)
            ...
            assert vat._mesh_stats(ue, mesh) == row['stats']
