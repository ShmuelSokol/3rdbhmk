...
def build(map_path, foot_location_cm, wind_location_cm):
    ...
    receipt = OUT / ('native-' + time.strftime('%Y%m%dT%H%M%S') + '-' + uuid.uuid4().hex[:8] + '.json')
...
def stop_audition():
    ...
    if ACTIVE is not None:
        ...
        # One invalid component or failed unregister must not skip other stops.
        try:
            ue.unregister_slate_post_tick_callback(state['handle'])
        except Exception as error:
            state['errors'].append('unregister: ' + str(error))
        for name, component in state['components'].items():
            try:
                component.stop()
            except Exception as error:
                state['errors'].append('stop ' + name + ': ' + str(error))
        try:
            state['components']['SW_park_ambience_wind'].set_volume_multiplier(.12)
        except Exception as error:
            state['errors'].append('restore wind gain: ' + str(error))
        (OUT / ('audition-' + state['stamp'] + '.json')).write_text(json.dumps(dict(
            status=('cleanup_errors_inspect_native_state' if state['errors'] else 'bounded_preview_stopped_not_listener_acceptance'), elapsed=time.monotonic()-state['start'],
...
def audition(map_path, seconds=120., bank='Stone'):
    ...
    ue = guard(map_path)
    expected = {item['name']: item for item in plan()}
    components = {}
    for actor in ue.get_editor_subsystem(ue.EditorActorSubsystem).get_all_level_actors():
        if TAG in [str(t) for t in actor.tags]:
            ...
            name = sound.get_name()
            require(name in expected and name not in components, 'Unexpected/duplicate review sound')
            require(not sound.get_editor_property('looping'), 'Looping source refused')
            require(not component.get_editor_property('auto_activate'), 'Autoplay source refused')
            components[name] = component
    require(len(components) == 13, 'Expected full saved review rig')
    state = dict(components=components, start=time.monotonic(), stamp=time.strftime('%Y%m%dT%H%M%S')+'-'+uuid.uuid4().hex[:8],
                 bank=bank, errors=[], next_step=2., side='L', previous={}, random=random.Random(771))
    def tick(_delta):
        if ACTIVE is not state:
            return # Failed unregister cannot restart audio after cleanup.
        try:
