"""Transient native GPU mantle reference-pose comparison; no asset/map saves."""
...
OUT = ROOT / 'SourceAssets/characters-review/CrowdMantleStudy05'
...
def run():
    ...
    try:
        ...
        source = OUT / 'V3_Pilgrim_Man_Standard_MantleStudy.glb'
        review = json.loads((OUT/'review.json').read_text())
        assert sha(source) == review['candidateSha256']
        baseline = ue.load_asset('/Game/MikdashV3/Characters/PilgrimRigV3/V3_Pilgrim_Man_Standard/V3_Pilgrim_Man_Standard/SkeletalMeshes/V3_Pilgrim_Man_Standard')
        assert baseline
        folder = '/Game/Characters/CrowdMantleStudy_' + stamp
