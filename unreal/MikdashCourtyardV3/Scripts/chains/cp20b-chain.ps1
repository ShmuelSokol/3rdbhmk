<#
  cp20b chain (precinct retaining faces: one-quarry tone V5 (bed-joint frame restored), close fade stays 1.0). Launched DETACHED.
  Steps: retone (-PMRetone=V5 -PMFarStrength=1.0) -> fresh-process verify -> cp20b cook (2 attempts)
  ...
  Progress: C:\Mikdash\Working-5.8\cp20b-chain.log   Status: C:\Mikdash\Working-5.8\cp20b-chain.status
#>
...
$log = "$root\cp20b-chain.log"
$statusFile = "$root\cp20b-chain.status"
...
function Busy-Reason {
    ...
    $s = Get-CimInstance Win32_Process -Filter "Name='powershell.exe' OR Name='pwsh.exe' OR Name='cmd.exe'" -EA SilentlyContinue |
         Where-Object { $_.ProcessId -ne $me -and $_.CommandLine -and
                        ...
                        $_.CommandLine -notmatch 'cp20b-chain\.ps1' }
...
try {
    ...
    if (-not $SkipRetone) {
        $s = Get-Date
        [void](Run-Engine 'macro-retoneV5' "-run=pythonscript -script=`"$script`" -PMRetone=V5 -PMFarStrength=1.0")
        $retone = Newest $macroReceipts 'precinct-macro-retone-V5-*.json' $s
    ...
    if ($retone) {
        $s = Get-Date
        [void](Run-Engine 'macro-verifyV5' "-run=pythonscript -script=`"$script`" -PMVerify=`"$($retone.FullName)`"")
    ...
    if (-not $SkipCook) {
        ...
        for ($attempt = 1; $attempt -le 2 -and -not $archive; $attempt++) {
            ...
            $p = Start-Process -FilePath 'powershell.exe' -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$proj\Checkpoint-Build.ps1`" -Label cp20b -WaitMinutes 120" -PassThru -WindowStyle Hidden
            if (-not $p.WaitForExit(150 * 60 * 1000)) { $p | Stop-Process -Force; Log 'cook timed out' }
            $job = Get-ChildItem -LiteralPath $root -Directory -Filter 'Checkpoint-cp20b-*' | Where-Object { $_.CreationTime -ge $s } | Sort-Object CreationTime | Select-Object -Last 1
        ...
        Log "ARCHIVE $archive"
        Set-Content -LiteralPath "$root\cp20b-chain.archive" -Value $archive -Encoding utf8
        Wait-Slot
        Log 'RUN capture'
        Status 'capturing'
        $p = Start-Process -FilePath 'powershell.exe' -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$proj\Scripts\capture_frame_precinct_macro.ps1`" -Archive `"$archive`" -Label cp20b -Views P1,P2,P3,02,07" -PassThru -WindowStyle Hidden
