"""Bounded continuous CharacterMovement probe: east gate stairs down and up.
...
output = root/'SourceAssets/runtime-review/ulam-connected-walk.json'
output.parent.mkdir(parents=True, exist_ok=True)
report = {'status':'running', 'scope':'Synthetic continuous CharacterMovement spawn to Ulam via northern altar bypass and return; no teleport; not physical keyboard or packaged acceptance', 'samples':[], 'reached':[], 'errors':[]}
started = time.monotonic()
handle = None
finished = False
phase = 0
route = [(1500,0,625),(1500,-1500,625),(-1300,-1500,625),(-1300,0,625),(-3025,0,925),(-1300,0,625),(-1300,-1500,625),(1500,-1500,625),(1500,0,625),(2100,0,500)]
last_sample = 0
...
def tick(delta):
    ...
    try:
        ...
        if not (-3200 < pos.x < 2300 and -1700 < pos.y < 200 and 500 < pos.z < 1100):
        ...
        report['spawnedMaxStepHeightCm']=movement.get_editor_property('max_step_height')
        assert report['spawnedMaxStepHeightCm']==55.0
        ...
        if unreal.GameplayStatics.get_time_seconds(world) < .05:
            ...
            return
        if elapsed < 2: return
        target, target_y, floor = route[phase]
        if abs(pos.x-target) < 30 and abs(pos.y-target_y) < 30:
            cap = pawn.get_component_by_class(unreal.CapsuleComponent)
            # Expected floor from source route contract.
            ...
            if phase == len(route):
                finish('completed_connected_ulam_route_requires_visual_review'); return
            target, target_y, floor = route[phase]
        ...
        dx,dy = target-pos.x,target_y-pos.y
