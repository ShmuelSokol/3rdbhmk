"""Bounded continuous CharacterMovement probe: source visitor lane north/south.
...
output = root/'SourceAssets/runtime-review/visitor-lane-continuous-walk.json'
output.parent.mkdir(parents=True, exist_ok=True)
report = {'status':'running', 'scope':'Synthetic continuous CharacterMovement along source visitor lane north/south/return; no teleport; not physical keyboard, access enforcement or packaged acceptance', 'samples':[], 'reached':[], 'errors':[]}
...
def tick(delta):
    ...
    try:
        ...
        if not (1900 < pos.x < 2300 and abs(pos.y) < 1800 and 450 < pos.z < 800):
        ...
        if unreal.GameplayStatics.get_time_seconds(world) < .05:
            ...
            return
        if elapsed < 2: return
        target_x,target_y = [(2100,-1500),(2100,1500),(2100,0)][phase]
        if math.hypot(pos.x-target_x,pos.y-target_y) < 30:
            ...
            if phase == 3:
                finish('completed_visitor_lane_north_south_return_requires_visual_review'); return
            target_x,target_y = [(2100,-1500),(2100,1500),(2100,0)][phase]
        ...
        dx,dy = target_x-pos.x,target_y-pos.y
