...
What it writes (SourceAssets/material-review/HerodianAshlarV4/):
...
(herodian-v4-import-*.json) unless --force is given.
"""
import argparse
import hashlib
import json
...
OUT = ROOT / 'SourceAssets' / 'material-review' / 'HerodianAshlarV4'
...
VARIANTS = {
    ...
    'Ashlar': dict(
        ...
        joint_cm=0.8, joint_depth_cm=1.05, arris_cm=0.26,
        ...
        boss_proud_cm=(1.40, 1.95), boss_edge_cm=0.40,
        ...
        base_offset_cm=0.30, tilt=0.0026, undulation_cm=0.100, boss_dish_cm=0.06, grain_cm=0.060,
        ...
        pore_density_per_cm2=0.0055, pore_radius_cm=(0.14, 0.90),
        weathering=(0.35, 1.0), weather_dark=0.150, weather_band=0.38, streak=0.045,
        bio_amount=0.24, bio_band=0.28, bio_grid=6, bio_tint=(0.72, 0.78, 0.68), bio_rough=0.05,
        ...
        base_srgb=(0.7026, 0.6106, 0.4867), value_jitter=0.125, gr_jitter=0.012, br_jitter=0.038,
        families=[('base', 0.44, 1.0, 0.0), ('cream', 0.20, 1.12, 0.018), ('honey', 0.22, 0.900, -0.058), ('greygold', 0.14, 0.950, 0.050)],
        margin_light=0.035, mottle=(0.045, 0.055, 0.052), joint_factor=0.46, joint_grey=0.22,
        rough={'boss': 0.560, 'margin': 0.615, 'joint': 0.80, 'chip': 0.75, 'pore': 0.80}, rough_jitter=0.028, rough_grain=0.022, rough_clamp=(0.45, 0.85),
        ao_gain=0.70, ao_floor=0.52, ao_wide_cm=6.0, ao_wide_gain=0.55),
    ...
    'Trim': dict(
        ...
        base_offset_cm=0.16, tilt=0.0018, undulation_cm=0.065, boss_dish_cm=0.04, grain_cm=0.042,
        ...
        pore_density_per_cm2=0.0034, pore_radius_cm=(0.11, 0.60),
        weathering=(0.20, 0.80), weather_dark=0.095, weather_band=0.34, streak=0.028,
        bio_amount=0.14, bio_band=0.24, bio_grid=8, bio_tint=(0.74, 0.80, 0.70), bio_rough=0.035,
        patina=0.028, patina_scale_cm=24.0, patina_fine=0.022, patina_fine_scale_cm=2.8,
        base_srgb=(0.7745, 0.6799, 0.5486), value_jitter=0.100, gr_jitter=0.010, br_jitter=0.028,
        families=[('base', 0.54, 1.0, 0.0), ('cream', 0.22, 1.09, 0.014), ('honey', 0.14, 0.925, -0.042), ('greygold', 0.10, 0.955, 0.042)],
        margin_light=0.028, mottle=(0.030, 0.036, 0.038), joint_factor=0.50, joint_grey=0.22,
        rough={'boss': 0.530, 'margin': 0.585, 'joint': 0.78, 'chip': 0.72, 'pore': 0.78}, rough_jitter=0.022, rough_grain=0.018, rough_clamp=(0.45, 0.85),
        ao_gain=0.75, ao_floor=0.58, ao_wide_cm=5.0, ao_wide_gain=0.55),
}
...
def main(argv=None):
    ...
    out = args.out
    receipts = list(out.glob('herodian-v4-import-*.json'))
    ...
    manifest = {'status': 'OFFLINE_GENERATED_NATIVE_IMPORT_PENDING', 'iteration': ITERATION,
                ...
                'generator': 'Scripts/create_herodian_ashlar_v4.py',
                ...
                'sources': 'SourceAssets/material-review/HerodianAshlarV4/sources.md (measured vs authored is marked there)',
    ...
    for name in (args.variant or sorted(VARIANTS)):
        ...
        for kind, rows in maps.items():
            path = out / ('T_HerodianV4_%s_%s%s.png' % (name, kind, suffix))
        ...
        for kind, rows in pv.items():
            path = out / 'previews' / ('T_HerodianV4_%s_%s_%d.png' % (name, kind, psize))
