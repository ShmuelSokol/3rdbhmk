...
r=json.loads((folder/'native-grain-pilot.json').read_text());assert r['status']=='failed_partial_review_required' and len(r['created'])==2
assert a.does_asset_exist(dest+'/M_InnerPaving_GrainReviewV1')
...
mat=a.load_asset(dest+'/M_InnerPaving_GrainReviewV1');assert isinstance(mat,unreal.Material)
r['recoveredPin']='ComponentMask uses unnamed input; previous two unconnected nodes retained in unassigned material'
try:
