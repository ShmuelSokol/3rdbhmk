"""Guarded native import of the photograph-calibrated MenorahV4 assembly (stepped hexagonal plinth
with eighteen relief panels, faceted shaft, six concentric-semicircle branches in two meshes, 42
ornaments, seven covered lamps, three-step tending stone) and its placement in the combined map at the
EXACT pose the MenorahV3 release occupies, replacing the seven RELEASE_MenorahV3_* actors.

V4 differs from V3 in geometry only: its proportions were measured from a Temple Institute photograph
(SourceAssets/vessels-review/MenorahV4/calibration.json - photograph-derived proportion, not a measured
survey of the vessel). Namespace, adapter, guards, winding check, clearance rules and receipt shape are
the reviewed V3 path unchanged.

What it does, in order (every number comes from Scripts/release_import_menorah_v4.spec.json, filled
from SourceAssets/vessels-review/MenorahV4/geometry-manifest.json and the V3 import receipt
native-import-20260908T003604089288Z.json):

  1. Guards: right project, no game world, no dirty packages, target namespace
     /Game/MikdashV3/MaterialReview/MenorahV4 absent (or present for -MenorahPlaceOnly), the seven
  ...
  2. Materials: the EXISTING gold material (first candidate on disk: M_AronStudyV3_Gold, else
     M_HeikhalKeilim_Gold) goes to EVERY slot of the six gold parts (SM_KeruvimStudyV1 lesson). The
     step stone gets the first existing stone candidate (MI_PBR_LimestoneTrim, ...); only if none
     exists is M_MenorahV4_Stone created inside the namespace. Recorded.
  3. Imports the seven OBJ parts through the reviewed legacy OBJ adapter path (FbxFactory, same
     settings as the keilim/reliefs/doors/vessels/shulchan/MenorahV4 imports, build_nanite False;
  ...
  5. Loads /Game/MikdashV3/IntegratedReviewV2/Maps/Walkthrough, refuses if any RELEASE_MenorahV4_*
     actor or the Release/MenorahV4 folder exists, checkpoints the .umap to
     C:/Mikdash/Working-5.8/ReviewCheckpoints/MenorahV4-<stamp>/, finds EXACTLY the seven
     RELEASE_MenorahV3_* actors (labels, folder Release/MenorahV3, MenorahV3 meshes and the pose the
     V3 receipt recorded), reads the Heikhal floor, and plans one world AABB per part at the SAME
     shared origin and yaw the V3 actors have - [-5330, 315.176, 925.00006] yaw -90 (lamp row
     north-south, local +Y = east). V4's fan is narrower than V3's, so the nearest lamp tip ends up
     about 134 cm from the south wall face instead of exactly 125 cm (2.5 amot, book p. 252); the
     spec's yRule value and both clearances are recorded and the guard only requires >= 125 cm.
  6. Clearance against every other mesh-bearing actor (release_place_assets rules, hollow unions
     decomposed), destroys the seven RELEASE_MenorahV3_* actors (poses recorded; the MenorahV3 assets
     stay on disk untouched), spawns RELEASE_MenorahV4_<Part> (BlockAll on Base and StepStone,
     NoCollision elsewhere, tag ReleaseMenorahV4), saves, reopens, reads back numerically and writes
     SourceAssets/vessels-review/MenorahV4/native-import-<stamp>.json (at start and in finally).
...
Commandlet invocation (serial; never while another native job is running; editor closed):
  ...
  "C:\\Program Files\\Epic Games\\UE_5.8\\Engine\\Binaries\\Win64\\UnrealEditor-Cmd.exe"
      ...
      -run=pythonscript
      -script="C:/Mikdash/Working-5.8/MikdashCourtyardV3/Scripts/release_import_menorah_v4.py"
      -EnablePlugins=GeometryScripting -unattended -nullrhi
      -abslog="C:/Mikdash/Working-5.8/Release-MenorahV4-01.log"
...
Offline (no engine):  python Scripts/release_import_menorah_v4.py  -> prints offline_check(): hashes,
manifest consistency, materials on disk, the planned world AABBs and the south-wall verdict per part.

NOT RUN BY THE AUTHORING PASS. This script launches the editor; the MenorahV4 authoring pass was
explicitly forbidden to launch Unreal, so only its offline_check() has been exercised.
"""
import hashlib
...
SPEC_PATH = ROOT / 'Scripts' / 'release_import_menorah_v4.spec.json'
...
def plan_parts(spec, helper, local_boxes, floor_top):
    ...
    # V4 stands exactly where the V3 release stands (the brief: same position and yaw). The book's own
    # rule - y = southWallY - 125 - halfSpan - is still computed and recorded, but V4's fan is narrower
    # than V3's, so following it would MOVE the vessel; the fixed origin is used and the resulting
    # clearance (larger than 125 cm) is checked against the minimum instead of against equality.
    rule_y = float(placement['southWallY']) - float(placement['nearestTipClearanceCm']) - half_span
    fixed = placement['fixedOrigin']
    origin = [float(fixed[0]), float(fixed[1]), floor_top - min_z]
    if abs(origin[0] - float(placement['x'])) > 1e-6:
        raise RuntimeError('fixedOrigin X %.4f disagrees with placement.x %.4f' % (origin[0], placement['x']))
    if abs(origin[2] - float(fixed[2])) > float(placement['originZToleranceCm']):
        raise RuntimeError('Floor-derived Z %.6f differs from the V3 actor Z %.6f' % (origin[2], fixed[2]))
    parts = {}
    ...
    if nearest_tip < float(placement['nearestTipClearanceCm']) - 1e-6:
        raise RuntimeError('Nearest lamp tip is %.4f cm from the south wall, closer than the book\'s %.1f cm'
                           % (nearest_tip, placement['nearestTipClearanceCm']))
    ...
    base = parts['SM_MenorahV4_Base']['plannedWorldBoundsCm']
    ...
    return {'origin': origin, 'yaw': yaw, 'halfSpanCm': half_span, 'parts': parts, 'placedParts': list(parts),
            'originSource': 'fixed at the MenorahV3 release pose (spec placement.fixedOrigin); Z re-derived '
                            'from the measured Heikhal floor top',
            'bookRuleYIfMoved': rule_y, 'bookRuleYNotApplied': 'V4 is narrower than V3, so the book rule would '
                               'shift the vessel; the brief keeps the V3 position',
...
def offline_check(spec=None, namespace_expected=None, parse_faces=True):
    ...
    if spec['removal']['location'] != spec['placement']['fixedOrigin']:
        raise RuntimeError('Removal pose and placement origin disagree; V4 must stand exactly where V3 stands')
    if float(spec['removal']['yaw']) != float(spec['placement']['yaw']):
        raise RuntimeError('Removal yaw and placement yaw disagree')
    if len(spec['removal']['expectedLabels']) != len(spec['source']['meshes']):
        raise RuntimeError('Removal expects %d actors but %d parts are placed'
                           % (len(spec['removal']['expectedLabels']), len(spec['source']['meshes'])))
...
class MenorahPlacement:
    ...
    def find_removal(self):
        """The seven RELEASE_MenorahV3_* actors this release replaces, with their poses recorded.

        Refuses unless every expected label is present exactly once, in the recorded folder, carrying a
        MenorahV3 mesh, at the recorded pose. Nothing is destroyed here; destruction happens in place()
        after the checkpoint and after the clearance check has passed.
        """
        removal = self.spec['removal']
        rows = []
        for label in removal['expectedLabels']:
            hits = [r for r in self.run.snapshot if r['label'] == label]
            if len(hits) != 1:
                raise RuntimeError('Removal target %s: %d actors with that label' % (label, len(hits)))
            row = hits[0]
            if row['folder'] != removal['folder']:
                raise RuntimeError('%s is in folder %s, expected %s' % (label, row['folder'], removal['folder']))
            if not row['meshes'] or not all((m or '').startswith(removal['meshPrefix']) for m in row['meshes']):
                raise RuntimeError('%s carries %s, expected meshes under %s' % (label, row['meshes'], removal['meshPrefix']))
            close, error = self.helper._pose_close(row['pose'], removal['location'], [0.0, removal['yaw'], 0.0],
                                                  removal['scale'], removal['poseToleranceCm'])
            if not close:
                raise RuntimeError('%s pose differs from the recorded V3 pose by %.4f' % (label, error))
            rows.append(row)
        extra = [r['label'] for r in self.run.snapshot
                 if r['label'] not in removal['expectedLabels']
                 and (r['folder'] == removal['folder']
                      or (r['meshes'] and all((m or '').startswith(removal['meshPrefix']) for m in r['meshes'])))]
        if extra:
            raise RuntimeError('Unexpected extra MenorahV3 release actors: %s' % extra[:10])
        return rows
    ...
    def place(self, meshes):
        ...
        removals = self.find_removal()
        self.receipt['removed'] = [{'label': r['label'], 'name': r['name'], 'folder': r['folder'], 'meshes': r['meshes'],
                                    'pose': r['pose'], 'worldBoundsCm': r['bounds'],
                                    'note': spec['removal']['assetsLeftInPlace']} for r in removals]
        ...
        excluded = {floor['label']} | {r['label'] for r in removals}
        ...
        try:
            for name in plan['placedParts']:
                ...
                info = meshes[name]
                tags = ['ReleaseMenorahV4', part['role'], name]
...
def run(import_only=False, allow_unverified_winding=False, place_only=False):
    ...
    try:
        ...
        if clashes:
            raise RuntimeError('Existing MenorahV4 release actors preserved; refusing duplicate placement: %s' % clashes[:10])
        ...
        receipt['status'] = 'menorah_v4_saved_reopened_menorah_v3_actors_removed_visual_acceptance_pending'
        return receipt
...
def _invoked_as_native_script():
    ...
    return 'release_import_menorah_v4.py' in command_line and ('-run=pythonscript' in command_line or '-executepythonscript' in command_line)
...
def _main():
    ...
    try:
        ...
        ue.log('release_import_menorah_v4: %s placed %s removed %s winding_ok %s' % (
            receipt['status'], receipt.get('placedActorCount'), receipt.get('removedActorCount'), receipt.get('windingVerifiedOutward')))
    except Exception as error:
        ue.log_error('release_import_menorah_v4 failed: ' + repr(error))
        raise
    finally:
