"""Guarded native import of the original ShulchanV2 assembly (table, rings, four snifim plates,
28 kanim, two loaf stacks, two bazichin) and its placement in the combined map, replacing the
crude REVIEW_HeikhalKeilim_SM_ShulchanStudy box (100 x 50 x 76 cm) at [-5300, -350, 925].

What it does, in order (every number comes from Scripts/release_import_shulchan.spec.json, filled
from SourceAssets/vessels-review/ShulchanV2/geometry-manifest.json and the keilim receipts):

  1. Guards: right project, no game world, no dirty packages, target namespace
     /Game/MikdashV3/MaterialReview/ShulchanV2 absent (or present for -ShulchanPlaceOnly), the seven
     frozen OBJ hashes and the manifest status intact.
  2. Materials: the EXISTING gold material (first candidate on disk) goes to every slot of the gold
     parts. The loaves get a bread material: an existing bread-like candidate if the spec lists one
     that exists, otherwise M_ShulchanV2_Bread is created inside the namespace by duplicating the gold
     material (EditorAssetSubsystem.duplicate_asset) and re-wiring BaseColor/Metallic/Roughness to warm
     matte constants (fallback: MaterialFactoryNew with the create_heikhal_keilim recipe). Recorded.
  3. Imports the seven OBJ parts through the reviewed legacy OBJ adapter path (FbxFactory, same
     settings as the keilim/reliefs/doors/vessels imports, build_nanite False). The files carry Y
  ...
  4. Winding check (GeometryScript, needs -EnablePlugins=GeometryScripting): signed volume under UE's
     ...
     passing check nothing is placed (override -ShulchanAllowUnverifiedWinding, recorded).
  5. Loads /Game/MikdashV3/IntegratedReviewV2/Maps/Walkthrough, refuses if any RELEASE_Shulchan_*
     actor or the Release/Shulchan folder exists, checkpoints the .umap to
     C:/Mikdash/Working-5.8/ReviewCheckpoints/Shulchan-<stamp>/, finds EXACTLY the old study actor
     (label, folder, mesh, pose as recorded in keilim-scale-20260907T171503Z.json), reads the Heikhal
     floor, plans one world AABB per part at the shared origin [-5300, -350, floorTop] yaw 0 (length
     east-west, +Y room side = south), applies the NORTH WALL GUARD: every placed part must keep
     >= 100 cm (2 amot; the book's 2.5-amot passage measured to the table) from the wall face Y -500.
     Parts that would violate it are dropped together with their dependants (kanim need the snifim)
     and the run falls back to the table-only set; the decision is recorded. Supports stand on the
     floor at the two LONG sides (north and south): book p. 250 diagram 22:17 shows the two 5-tefach
     plates face-on side by side with the rod ends as dots, and the Temple Institute columns stand in
     pairs at the long sides; recorded in the spec.
  6. Clearance against every other mesh-bearing actor (release_place_assets rules, hollow unions
     decomposed), destroys the old study actor (pose recorded), spawns RELEASE_Shulchan_<Part>
     (NoCollision, tag ReleaseShulchanV2), saves, reopens, reads back numerically and writes
     SourceAssets/vessels-review/ShulchanV2/native-import-<stamp>.json (at start and in finally).
...
Commandlet invocation (serial; never while another native job is running):
  ...
  "C:\\Program Files\\Epic Games\\UE_5.8\\Engine\\Binaries\\Win64\\UnrealEditor-Cmd.exe"
      ...
      -run=pythonscript
      -script="C:/Mikdash/Working-5.8/MikdashCourtyardV3/Scripts/release_import_shulchan.py"
      -EnablePlugins=GeometryScripting -unattended -nullrhi
      -abslog="C:/Mikdash/Working-5.8/Release-Shulchan-01.log"

Optional switches (read from the engine command line):
  -ShulchanImportOnly               import + materials + winding check, no map change.
  -ShulchanPlaceOnly                skip the import; the seven meshes must already exist (re-verified).
  -ShulchanAllowUnverifiedWinding   place even if the winding check is unavailable/failed.
  -ShulchanTableOnly                place only the table-only set (no snifim, no kanim) regardless of clearance.

Offline (no engine):  python Scripts/release_import_shulchan.py  -> prints offline_check(): hashes,
manifest consistency, materials on disk, the planned world AABBs, the wall-clearance verdict per part.
"""
import hashlib
...
SPEC_PATH = ROOT / 'Scripts' / 'release_import_shulchan.spec.json'
...
def translate(box, location):
    return {'min': [box['min'][i] + location[i] for i in range(3)], 'max': [box['max'][i] + location[i] for i in range(3)]}
...
def plan_parts(spec, local_boxes, floor_top, table_only=False):
    """World AABB per part at the shared origin; wall-clearance verdict; the set actually placed.

    local_boxes: {partName: canonical local box} (manifest or imported asset).
    """
    placement = spec['placement']
    origin = [placement['origin'][0], placement['origin'][1], floor_top]
    yaw = float(placement['yaw'])
    if yaw != 0.0:
        raise RuntimeError('Plan assumes yaw 0 (length east-west); spec yaw is %r' % yaw)
    wall = placement['northWall']
    parts = {}
    for record in spec['source']['meshes']:
        name = record['name']
        world = translate(local_boxes[name], origin)
        clearance = world['min'][1] - wall['faceY']
        parts[name] = {'label': placement['labelPrefix'] + record['part'], 'part': record['part'], 'role': record['role'],
                       'location': origin, 'rotation': [0.0, yaw, 0.0], 'scale': [1.0, 1.0, 1.0],
                       'plannedWorldBoundsCm': world, 'northWallClearanceCm': clearance,
                       'wallClearanceOk': clearance >= wall['minClearanceCm'] - 1e-9,
                       'dependsOn': record.get('dependsOn', [])}
    violating = {n for n, p in parts.items() if not p['wallClearanceOk']}
    dropped = set(violating)
    if table_only:
        dropped |= {n for n in parts if n not in spec['placement']['tableOnlyParts']}
    changed = True
    while changed:
        changed = False
        for name, part in parts.items():
            if name not in dropped and any(dep in dropped for dep in part['dependsOn']):
                dropped.add(name)
                changed = True
    placed = [n for n in parts if n not in dropped]
    if violating and not table_only:
        # The brief's rule: if the supports would violate the wall clearance, place the table only.
        placed = [n for n in placed if n in spec['placement']['tableOnlyParts']]
        dropped = {n for n in parts if n not in placed}
    for name in parts:
        parts[name]['placed'] = name in placed
        parts[name]['dropReason'] = None if name in placed else (
            'north wall clearance %.1f cm < %.0f cm' % (parts[name]['northWallClearanceCm'], wall['minClearanceCm']) if name in violating
            else 'table-only switch' if table_only else 'dependency or table-only fallback after a wall-clearance violation')
    essential = [n for n in spec['placement']['tableOnlyParts'] if n not in placed]
    if essential:
        raise RuntimeError('Even the table-only set violates the wall guard: %s' % essential)
    envelope = placement['roomEnvelope']
    for name in placed:
        if not box_inside(parts[name]['plannedWorldBoundsCm'], envelope):
            raise RuntimeError('%s planned AABB leaves the Heikhal envelope: %r' % (name, parts[name]['plannedWorldBoundsCm']))
    union = {'min': [min(parts[n]['plannedWorldBoundsCm']['min'][i] for n in placed) for i in range(3)],
             'max': [max(parts[n]['plannedWorldBoundsCm']['max'][i] for n in placed) for i in range(3)]}
    return {'origin': origin, 'yaw': yaw, 'parts': parts, 'placedParts': placed, 'droppedParts': sorted(dropped),
            'mode': 'full_assembly' if not dropped else 'table_only_fallback', 'assemblyWorldBoundsCm': union,
            'assemblyNorthWallClearanceCm': union['min'][1] - wall['faceY'],
            'supportsSide': placement['supportsSide']}


def offline_check(spec=None, namespace_expected=None, table_only=False, parse_faces=True):
    ...
    for record in source['meshes']:
        ...
        row = {'name': record['name'], 'part': record['part'], 'role': record['role'], 'sha256': actual_sha, 'triangles': record['triangles']}
        if parse_faces:
    ...
    if removal['location'] != spec['placement']['origin'] or removal['yaw'] != spec['placement']['yaw']:
        raise RuntimeError('Removal pose and placement origin disagree; the new assembly replaces the study in place')
    report['plan'] = plan_parts(spec, local_boxes, spec['placement']['floor']['topZ'], table_only)
...
def bread_material(ue, spec, receipt):
    """Existing bread-like candidate, else a warm matte constant material duplicated from the gold one."""
    ...
    if assets.does_asset_exist(bread['createPath']):
        ...
        receipt['materials']['bread'].update({'assigned': bread['createPath'], 'kind': 'bread_constant_reused', 'created': False})
        return material
    method = None
    material = None
    try:
    ...
    except Exception as error:  # noqa: BLE001 - fall back to the factory recipe used by create_heikhal_keilim
    ...
    receipt['materials']['bread'].update({'assigned': _asset_path(material), 'kind': 'bread_constant_created', 'created': True,
...
def load_saved_mesh(ue, spec, record, receipt):
    ...
    info = {'asset': _asset_path(mesh), 'file': None, 'triangles': triangles, 'vertices': mesh.get_num_vertices(0),
            'localBoundsCm': box, 'materialSlots': slots, 'material': material_path, 'role': record['role'], 'reused': True,
...
class ShulchanPlacement:
    ...
    def find_removal(self):
        ...
        if others:
            raise RuntimeError('Unexpected extra actors carrying the study mesh: %s' % others[:10])
        return row
    ...
    def place(self, meshes, table_only):
        ue = self.ue
        spec = self.spec
        ...
        plan = plan_parts(spec, local_boxes, floor['bounds']['max'][2], table_only)
        self.receipt['plan'] = plan
        self.receipt['wallGuard'] = {'northWallFaceY': spec['placement']['northWall']['faceY'],
                                     'minClearanceCm': spec['placement']['northWall']['minClearanceCm'],
                                     'assemblyClearanceCm': plan['assemblyNorthWallClearanceCm'], 'mode': plan['mode'],
                                     'droppedParts': plan['droppedParts'],
                                     'perPart': {n: p['northWallClearanceCm'] for n, p in plan['parts'].items()}}
        if plan['droppedParts']:
            self.receipt['limitations'].append('Table-only fallback: %s not placed (%s)' % (
                plan['droppedParts'], {n: plan['parts'][n]['dropReason'] for n in plan['droppedParts']}))
        ...
        try:
            for name in plan['placedParts']:
                ...
                info = meshes[name]
                tags = ['ReleaseShulchanV2', part['role'], name]
                ...
                records.append({'label': part['label'], 'kind': 'StaticMeshActor', 'part': part['part'], 'role': part['role'], 'mesh': info['asset'],
                                ...
                                'collisionProfile': spec['collisionProfile'], 'plannedWorldBoundsCm': part['plannedWorldBoundsCm'],
                                'placedWorldBoundsCm': placed_bounds, 'boundsErrorCm': bounds_error,
                                'northWallClearanceCm': part['northWallClearanceCm'], 'actor': actor})
        except Exception:
...
def run(import_only=False, allow_unverified_winding=False, place_only=False, table_only=False):
    import unreal as ue
    if import_only and place_only:
        raise RuntimeError('-ShulchanImportOnly and -ShulchanPlaceOnly are mutually exclusive')
    spec = load_spec()
    helper = load_placement_helper(spec)
    offline = offline_check(spec, namespace_expected=place_only, table_only=table_only)
    ...
    if not place_only and namespace_exists:
        raise RuntimeError('Existing native namespace preserved (use -ShulchanPlaceOnly): ' + spec['source']['namespace'])
    ...
    receipt = {
        ...
        'switches': {'importOnly': import_only, 'placeOnly': place_only, 'allowUnverifiedWinding': allow_unverified_winding, 'tableOnly': table_only},
        'mode': 'place_only_reusing_saved_meshes' if place_only else 'import_and_place',
        'supportsSide': spec['placement']['supportsSide'],
    ...
    try:
        ...
        if not all_passed and spec['windingCheck']['requireForPlacement'] and not allow_unverified_winding:
            ...
            receipt['limitations'].append('Placement skipped: winding check did not pass or was unavailable; rerun with '
                                          '-EnablePlugins=GeometryScripting -ShulchanPlaceOnly or inspect before -ShulchanAllowUnverifiedWinding')
            return receipt
        if not all_passed:
        ...
        if clashes:
            raise RuntimeError('Existing shulchan release actors preserved; refusing duplicate placement: %s' % clashes[:10])
        receipt['actorCountBefore'] = len(run_state.snapshot)
        write()

        # -- 4. remove, place, save, reopen, read back --------------------------------------
        placement = ShulchanPlacement(ue, spec, helper, run_state, receipt)
        records = placement.place(meshes, table_only)
        ...
        for record in records:
            ...
            entry = {'label': record['label'], 'folder': row['folder'], 'meshPath': row['meshes'], 'pose': row['pose'],
                     ...
                     'northWallClearanceCm': row['bounds']['min'][1] - spec['placement']['northWall']['faceY'],
            ...
            if entry['northWallClearanceCm'] < spec['placement']['northWall']['minClearanceCm'] - 1e-6:
        ...
        receipt['status'] = ('shulchan_v2_saved_reopened_old_study_removed_visual_acceptance_pending' if receipt['plan']['mode'] == 'full_assembly'
                             else 'shulchan_v2_table_only_saved_reopened_old_study_removed_supports_skipped')
        return receipt
...
def _invoked_as_native_script():
    ...
    return 'release_import_shulchan.py' in command_line and ('-run=pythonscript' in command_line or '-executepythonscript' in command_line)
...
def _main():
    ...
    try:
        receipt = run(import_only='-shulchanimportonly' in tokens, allow_unverified_winding='-shulchanallowunverifiedwinding' in tokens,
                      place_only='-shulchanplaceonly' in tokens, table_only='-shulchantableonly' in tokens)
        ue.log('release_import_shulchan: %s placed %s removed %s winding_ok %s' % (
            receipt['status'], receipt.get('placedActorCount'), receipt.get('removedActorCount'), receipt.get('windingVerifiedOutward')))
    except Exception as error:
        ue.log_error('release_import_shulchan failed: ' + repr(error))
        raise
    finally:
