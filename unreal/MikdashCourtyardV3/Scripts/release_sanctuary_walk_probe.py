"""Bounded sanctuary architectural-review PIE probe; not ritual eligibility.
Derived from release_walk_probe.py. Dedicated editor -ExecCmds only, no commandlet.
One initial unswept PIE pawn placement at [-3450,0,925+capsuleHalfHeight+2];
no teleport, flying, collision disable or map save after this explicit setup.
Existing static open doorway and NoCollision curtain are architectural omissions,
not tested functional door/curtain interaction. Native movement acceptance pending.
MIKDASH_WALK_QUIT_EDITOR=1 only in a dedicated root-owned process; timeout <=90s.
Receipt: SourceAssets/IntegratedReviewV2/release-sanctuary-walk-<UTC>.json.
"""
import hashlib
import json
import math
...
def build_routes():
    ...
    return [dict(name='sanctuary_round_trip',
      ...
      envelope=dict(x=(-5960,-3350),y=(-180,200),z=(900,1150)),
      legs=[('enter Heikhal',-4100.,0.,925.,True),
            ('altar east detour',-4350.,120.,925.,True),
            ('altar west detour',-5000.,120.,925.,True),
            ('return to doorway axis',-5300.,0.,925.,True),
            ('Kodesh east landing',-5900.,0.,925.,True),
            ('return through Kodesh doorway',-5300.,0.,925.,True),
            ('return altar west detour',-5000.,120.,925.,True),
            ('return altar east detour',-4350.,120.,925.,True),
            ('return Heikhal east',-4100.,0.,925.,True),
            ('exit Heikhal doorway',-3450.,0.,925.,True)])]
...
report = {
    ...
    'routes': [dict(name=r['name'], source=r['source'], envelope=r['envelope'],
               ...
               for r in ROUTES],
    'protectedBefore': protected_before, 'altarBounds': altar_actors, 'initialPlacement': None, 'pawn': None, 'resumeCalls': 0, 'events': [], 'reached': [], 'passThrough': [], 'samples': [], 'errors': [],
}
...
def finish(status):
    if state['finished']:
        return
    state['finished'] = True
    state['requested_status']=status
    report['status'] = 'awaiting_pie_teardown'
    ...
    try:
        ...
        report['protectedAfter']={str(p):sha(p) for p in protected_files}
        report['protectedUnchanged']=report['protectedAfter']==protected_before
        if not report['mapBytesUnchanged'] or not report['protectedUnchanged']:
            report['status']='failed_map_hash_changed';report['errors'].append('Protected/main map bytes changed')
    except Exception as exc:
    ...
    report['pieTeardownConfirmed']=False
    write_report()
    unreal.log('release_sanctuary_walk_probe: ' + status + ' -> ' + str(output))


def quit_tick(delta):
    elapsed=time.monotonic()-state['end_requested_at']
    ...
    try:
        ...
        report['errors'].append(str(exc));report['status']='failed_cleanup_or_verification'
    write_report()
    if QUIT_EDITOR:unreal.SystemLibrary.quit_editor()
...
def advance_leg(pos, foot, now):
    ...
    if state['leg'] >= len(route['legs']):
        ...
        if state['route'] >= len(ROUTES):
            finish('completed_sanctuary_round_trip_architectural_review_only')
            return True
...
def tick(delta):
    ...
    try:
        ...
        if state['pawn_path'] is None:
            ...
            report['initialPlacement']=[placed.x,placed.y,placed.z]
        ...
        if capsule.get_collision_enabled() == unreal.CollisionEnabled.NO_COLLISION:
            raise RuntimeError('Capsule collision disabled')
        ...
        if now - state['last_sample'] >= SAMPLE_SECONDS:
            ...
            sample = dict(seconds=round(elapsed, 3), gameTimeSeconds=game_time, positionCm=xyz,
                          ...
                          capsuleCollision=str(capsule.get_collision_enabled()), capsuleProfile=str(capsule.get_collision_profile_name()), paused=paused, menuOpen=menu_open, route=route['name'], leg=state['leg'],
...
try:
    ...
    finish('failed_start')
    raise
unreal.log('release_sanctuary_walk_probe started; receipt ' + str(output))
