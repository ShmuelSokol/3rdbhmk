...
import sys,types
m=types.ModuleType('mikdash_exposure_review');sys.modules[m.__name__]=m
exec('''
...
path=Path(unreal.Paths.project_dir(),'SourceAssets/visual-review/exposure-comparison.json')
...
def stop():
 ...
 unreal.log('EXPOSURE_REVIEW_RESTORED')
def tick(delta):
 global phase
 try:
...
try:
 ...
 e.set_level_viewport_camera_info(unreal.Vector(2100,-1500,668),unreal.Rotator(pitch=0,yaw=170,roll=0));record('baseline')
except Exception:
 stop();raise
''',m.__dict__)
