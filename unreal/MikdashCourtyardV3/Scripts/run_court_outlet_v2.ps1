param([switch]$Apply, [switch]$CollisionApply)
$ErrorActionPreference = 'Stop'
if($Apply -and $CollisionApply){throw 'Choose import or collision mode'}
...
$log="$folder/outlet-run-$stamp.log"
$receipt="$folder/outlet-run-$stamp.json"
if(Test-Path -LiteralPath $receipt){throw 'Fresh audit receipt required'}
$before=@(Get-ChildItem -LiteralPath $folder -Filter 'native-*.json' -File | Where-Object {$_.Name -notlike 'outlet-run-*'} | Select-Object -ExpandProperty FullName)
$record=[ordered]@{status='starting';scope='Fresh two-asset import or read-only verification; no maps';apply=[bool]$Apply;collisionApply=[bool]$CollisionApply;startedUtc=$stamp;log=$log;minimumStartCommitBytes=6GB;maximumPrivateBytes=4GB;reserveCommitBytes=2GB;peakPrivateBytes=0}
...
try {
    ...
    $outputs=@(Get-ChildItem -LiteralPath $folder -Filter 'native-*.json' -File | Where-Object {$_.Name -notlike 'outlet-run-*'} | Where-Object {$_.FullName -notin $before})
    ...
    $expectedStatus=if($Apply -or $CollisionApply){'imported-needs-fresh-verification'}else{'verified-fresh-native'}
