...
$folder="$root/SourceAssets/characters-review/CrowdMantleStudy05"
New-Item -ItemType Directory -Path $folder -Force | Out-Null
$log="$folder/mantle-render-run-$stamp.log"
$receipt="$folder/mantle-render-run-$stamp.json"
if(Test-Path -LiteralPath $receipt){throw 'Fresh audit receipt required'}
$before=@(Get-ChildItem -LiteralPath $folder -Filter 'render-*.json' -File | Where-Object {$_.Name -notlike 'mantle-render-run-*'} | Select-Object -ExpandProperty FullName)
...
try {
    ...
    $outputs=@(Get-ChildItem -LiteralPath $folder -Filter 'render-*.json' -File | Where-Object {$_.Name -notlike 'mantle-render-run-*'} | Where-Object {$_.FullName -notin $before})
    if($outputs.Count -ne 1){throw 'Expected one fresh mantle candidate receipt'}
    $result=Get-Content -LiteralPath $outputs[0].FullName -Raw | ConvertFrom-Json
    $expectedStatus='captured-review-pending'
    if($result.status -ne $expectedStatus){throw 'Mantle candidate operation did not complete'}
