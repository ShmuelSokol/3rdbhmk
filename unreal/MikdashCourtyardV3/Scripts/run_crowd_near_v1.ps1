...
$folder="$root/SourceAssets/perf-review/crowd-vat/NearV1"
New-Item -ItemType Directory -Path $folder -Force | Out-Null
$log="$folder/near-run-$stamp.log"
$receipt="$folder/near-run-$stamp.json"
if(Test-Path -LiteralPath $receipt){throw 'Fresh audit receipt required'}
$before=@(Get-ChildItem -LiteralPath $folder -Filter 'native-*.json' -File | Where-Object {$_.Name -notlike 'near-run-*'} | Select-Object -ExpandProperty FullName)
$record=[ordered]@{status='starting';scope='Fresh full-source single crowd VAT candidate or readback; no maps';build=[bool]$Build;startedUtc=$stamp;log=$log;minimumStartCommitBytes=6GB;maximumPrivateBytes=4GB;reserveCommitBytes=2GB;peakPrivateBytes=0}
...
try {
    ...
    $outputs=@(Get-ChildItem -LiteralPath $folder -Filter 'native-*.json' -File | Where-Object {$_.Name -notlike 'near-run-*'} | Where-Object {$_.FullName -notin $before})
