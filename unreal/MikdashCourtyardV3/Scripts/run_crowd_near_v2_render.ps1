...
$log="$folder/near-render-run-$stamp.log"
$receipt="$folder/near-render-run-$stamp.json"
if(Test-Path -LiteralPath $receipt){throw 'Fresh audit receipt required'}
$before=@(Get-ChildItem -LiteralPath $folder -Filter 'render-*.json' -File | Where-Object {$_.Name -notlike 'near-render-run-*'} | Select-Object -ExpandProperty FullName)
...
try {
    ...
    $outputs=@(Get-ChildItem -LiteralPath $folder -Filter 'render-*.json' -File | Where-Object {$_.Name -notlike 'near-render-run-*'} | Where-Object {$_.FullName -notin $before})
