param([switch]$Build,[ValidateSet('02','03')][string]$Study='02')
...
$folder="$root/SourceAssets/perf-review/crowd-vat/ResidentRuntime$Study"
...
$record=[ordered]@{status='starting';scope='Separate resident live movement review map; no production-map edits';build=[bool]$Build;startedUtc=$stamp;log=$log;minimumStartCommitBytes=6GB;maximumPrivateBytes=4GB;reserveCommitBytes=2GB;peakPrivateBytes=0}
