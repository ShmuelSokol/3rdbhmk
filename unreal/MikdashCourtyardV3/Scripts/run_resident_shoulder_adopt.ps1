...
$folder="$root/SourceAssets/characters-review/ResidentShoulderAdopt01"
New-Item -ItemType Directory -Path $folder -Force | Out-Null
$log="$folder/resident-shoulder-run-$stamp.log"
$receipt="$folder/resident-shoulder-run-$stamp.json"
if(Test-Path -LiteralPath $receipt){throw 'Fresh audit receipt required'}
$before=@(Get-ChildItem -LiteralPath $folder -Filter 'shoulder-adopt-*.json' -File | Where-Object {$_.Name -notlike 'resident-shoulder-run-*'} | Select-Object -ExpandProperty FullName)
$record=[ordered]@{status='starting';scope='Backed-up five resident sleeve repairs or fresh readback; no map edits';startedUtc=$stamp;log=$log;minimumStartCommitBytes=6GB;maximumPrivateBytes=4GB;reserveCommitBytes=2GB;peakPrivateBytes=0}
...
try {
    ...
    $outputs=@(Get-ChildItem -LiteralPath $folder -Filter 'shoulder-adopt-*.json' -File | Where-Object {$_.Name -notlike 'resident-shoulder-run-*'} | Where-Object {$_.FullName -notin $before})
