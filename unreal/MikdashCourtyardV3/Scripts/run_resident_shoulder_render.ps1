...
$before=@(Get-ChildItem -LiteralPath $folder -Filter 'render-*.json' -File | Where-Object {$_.Name -notlike 'shoulder-render-run-*'} | Select-Object -ExpandProperty FullName)
...
try {
    ...
    $outputs=@(Get-ChildItem -LiteralPath $folder -Filter 'render-*.json' -File | Where-Object {$_.Name -notlike 'shoulder-render-run-*'} | Where-Object {$_.FullName -notin $before})
