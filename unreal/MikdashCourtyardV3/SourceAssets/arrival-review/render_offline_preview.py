"""Stdlib depth-buffer geometry preview, not a native lighting/visual acceptance."""
import json
import math
...
FOLDER = Path(__file__).resolve().parent / 'BusStudyV1'
data = json.loads((FOLDER/'bus-editable.mesh.json').read_text())
palette = json.loads((FOLDER/'offline-checks.json').read_text())['palette']
w,h = 1300,760
pixels = bytearray(bytes((196,203,208))*w*h)
depth = [-1e30]*(w*h)
def project(v):
    x,y,z=v
    return (650+.82*x-.7*y,465+.18*x+.34*y-.95*z,.665*x+.779*y+.4048*z)
for p in data['parts']:
    for f in p['triangles']:
        raw = [p['vertices'][i] for i in f]
        a,b,c=raw
