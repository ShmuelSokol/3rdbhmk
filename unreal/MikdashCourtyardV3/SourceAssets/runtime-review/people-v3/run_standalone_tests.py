"""people-v3 copy: compile and run every Plugins/MikdashRuntime/Tests/*.cpp standalone with installed MSVC.
...
# PeopleDirectoryTest reads the real authored directory through the shipped parser when
# this is set, so the test that guards the schema also guards the file the game loads.
PEOPLE = ROOT / 'SourceAssets/runtime-review/people-v3/people.json'
...
def main():
    ...
    report = dict(stamp=datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'),
                  ...
                  authoredDirectory=str(PEOPLE.relative_to(ROOT)),
                  authoredDirectorySha256=sha(PEOPLE) if PEOPLE.is_file() else None,
                  results=[], allPassed=True)
    env['MIKDASH_PEOPLE_JSON'] = str(PEOPLE)
    for source in sorted(TESTS.glob('*.cpp')):
        ...
        if compiled.returncode != 0:
            ...
            run = subprocess.run([str(exe)], capture_output=True, text=True, env=env, cwd=str(build), timeout=120)
